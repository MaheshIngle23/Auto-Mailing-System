...
public class UserService {
	...
	public String login(LoginDto loginDto) {
		Student stud = studentRepository.findByEmail(loginDto.getEmail()).orElse(null);

		if (stud == null) {
			return "User not found with this email: " + loginDto.getEmail();
		} else if (!passwordEncoder.matches(loginDto.getPassword(), stud.getPassword())) {
	...
	public String sendMailWithAttachment(String email, String password)
			throws MessagingException, IllegalStateException, IOException {
		@SuppressWarnings("unused")
		Student stud = studentRepository.findByEmail(email)
				.orElseThrow(() -> new RuntimeException("User not found with this email: " + email));
		emailUtil.sendAttachment(email, password);
